...
function LineRows({ list, indent = false, pal }: { list: ClaimLineItemForPDF[]; indent?: boolean; pal: PdfPalette }) {
      ...
      {list.map((li, i) => {
        ...
        return (
          <View key={li.id} style={[s.row, i % 2 !== 0 ? s.rowAlt : {}]} wrap={false}>
            ...
            <Text style={[s.td, { width: 65, textAlign: 'right', color: pal.accent }]}>
              {li.this_pct > 0 ? `${li.this_pct}%` : '—'}
            </Text>
            <Text style={[s.td, { width: 75, textAlign: 'right', fontFamily: 'Helvetica-Bold', color: pal.accent }]}>
      ...
      <Page size="A4" style={s.page}>
        ...
        {isRetention ? (
          ...
          <>
            ...
            <LineRows pal={palette} list={freeItems} />
            ...
            {sections.map(sec => {
              ...
              return (
                <View key={sec.id}>
                  ...
                  <LineRows pal={palette} list={secItems} indent />
                </View>
              )
            })}
          </>

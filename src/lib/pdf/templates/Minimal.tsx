...
export function MinimalTemplate({ project, client, lineItems, type, theme, vatRate = 15, depositPct = 50, amountPaid = 0, footerText, logoUrl, businessName, businessAddress, vatNumber, companyReg, bankName, bankAccount, bankBranch, termsConditions, quotedDate, validityDays, paymentTerms, leadTime, itemImages }: TemplateProps) {
    ...
    <Document>
      <Page size="A4" style={{ fontFamily: 'Helvetica', fontSize: 9, color: theme.text, padding: 56, flexDirection: 'column' }}>
        <View style={{ flex: 1 }}>
          ...
          {type === 'quote' ? (
            <View style={{ marginTop: 28 }}>
              <Text style={{ fontSize: 7, color: theme.muted, letterSpacing: 1.5, marginBottom: 8 }}>ACCEPTANCE</Text>
              <Text style={{ fontSize: 7, color: theme.muted, marginBottom: 20, lineHeight: 1.7 }}>

...
export function ClassicTemplate({ project, client, lineItems, type, theme, vatRate = 15, depositPct = 50, amountPaid = 0, footerText, logoUrl, businessName, businessAddress, vatNumber, companyReg, bankName, bankAccount, bankBranch, termsConditions, quotedDate, invoicedDate, validityDays, paymentTerms, leadTime, itemImages, acceptance }: TemplateProps) {
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16 }}>
            <View style={{ flex: 1, paddingRight: 16 }}>
              {logoUrl
                ? <View style={{ alignItems: 'flex-start', marginBottom: 8 }}><Image src={logoUrl} style={{ maxWidth: 300, maxHeight: 72, objectFit: 'contain' }} /></View>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12, borderBottomWidth: 0.5, borderBottomColor: theme.border, paddingBottom: 8 }}>
            {client ? (
              <View style={{ flex: 1 }}>
                ...
                {client.email && <Text style={{ fontSize: 9, color: theme.text }}>{client.email}</Text>}
              </View>
            ) : <View style={{ flex: 1 }} />}
              ...
              {type === 'quote' ? (
                ...
                <>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
                    ...
                    <Text style={{ fontSize: 8, color: theme.accent, fontFamily: 'Helvetica-Bold' }}>-{formatZAR(depositReceived)}</Text>
                  </View>
                  <View style={{ borderTopWidth: 0.5, borderTopColor: theme.border, marginTop: 5, paddingTop: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ fontSize: 9, fontFamily: 'Helvetica-Bold', color: theme.primary }}>AMOUNT DUE</Text>
          ...
          {/* Acceptance block — heading, wording and rules all come from studio settings */}
          {type === 'quote' && acceptance ? (
            <View style={{ marginTop: 16, borderWidth: 1, borderColor: theme.border, borderRadius: 4, padding: 12 }}>
              {acceptance.heading ? (
                <Text style={{ fontSize: 7, fontFamily: 'Helvetica-Bold', color: theme.muted, marginBottom: 6, borderBottomWidth: 0.5, borderBottomColor: theme.border, paddingBottom: 4 }}>{acceptance.heading}</Text>
              ) : null}
              {acceptance.body ? (
                <Text style={{ fontSize: 7, color: theme.text, marginBottom: 12, lineHeight: 1.5 }}>{acceptance.body}</Text>
              ) : null}

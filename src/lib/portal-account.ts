...
export const resolvePortalAccount = cache(async (userId: string): Promise<PortalAccount | null> => {
  const [{ data: ownerAccount }, { data: membership }] = await Promise.all([
    supabaseAdmin
      ...
      .select('id, email, company_name, plan, subscription_status, trial_ends_at, supplier_category')
      .eq('auth_user_id', userId)
      .maybeSingle(),
    supabaseAdmin
      .from('portal_org_members')
      .select('portal_account_id, account:supplier_portal_accounts(id, email, company_name, plan, subscription_status, trial_ends_at, supplier_category)')
      .eq('auth_user_id', userId)

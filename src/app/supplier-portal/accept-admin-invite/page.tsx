...
function AcceptAdminContent() {
  ...
  const [info, setInfo] = useState<{ name?: string; company?: string; email?: string }>({})
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus('done')
    setTimeout(() => router.push('/supplier-portal/home'), 2000)
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: S.bg }}>
      <div className="w-full max-w-sm rounded-2xl overflow-hidden" style={{ background: S.card, border: `1px solid ${S.border}`, boxShadow: '0 16px 48px rgba(0,0,0,0.1)' }}>
        ...
        <div className="p-6">
          {status === 'loading' && <div className="flex items-center justify-center py-8"><Loader2 className="animate-spin" style={{ color: S.accent }} size={24} /></div>}
          {status === 'done' && (
            <div className="flex flex-col items-center py-6 gap-3">
              <CheckCircle2 size={40} style={{ color: S.green }} />
              <p className="font-bold text-sm" style={{ color: S.text }}>Account created!</p>
              <p className="text-xs text-center" style={{ color: S.muted }}>Taking you to the dashboard…</p>
            </div>
          )}
          {status === 'error' && (
            <div className="flex flex-col items-center py-6 gap-3">
          ...
          {(status === 'form' || status === 'submitting') && (
            <form onSubmit={e => void handleSubmit(e)} className="space-y-4">
              ...
              {error && <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs" style={{ background: '#FEF2F2', color: S.danger }}><AlertCircle size={12} />{error}</div>}

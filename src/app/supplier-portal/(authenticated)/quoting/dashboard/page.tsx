...
export default async function QuotingDashboardPage() {
  ...
  if (planRank(account.plan) === 1) {
    ...
    const [{ data: staffData }, { data: todayPunchData }, { data: weekPunchData }] = await Promise.all([
      ...
      supabaseAdmin.from('elec_time_punches').select('id, staff_id, punch_type, punched_at, latitude, longitude, notes, job_id').eq('portal_account_id', account.id).gte('punched_at', todayStart.toISOString()).order('punched_at', { ascending: false }),
      supabaseAdmin.from('elec_time_punches').select('id, staff_id, punch_type, punched_at, job_id').eq('portal_account_id', account.id).gte('punched_at', weekStart.toISOString()).order('punched_at'),
    ])

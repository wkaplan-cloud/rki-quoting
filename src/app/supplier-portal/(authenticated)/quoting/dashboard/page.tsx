...
export default async function QuotingDashboardPage() {
  ...
  const [{ data: quotesRaw }, { data: claimsRaw }, { data: jobCardsRaw }] = await Promise.all([
    supabaseAdmin
      .from('elec_quotes')
      .select('id, quote_number, project_name, status, contract_type, expected_completion_date, client:elec_clients(client_name), line_items:elec_quote_line_items(quoted_quantity, quoted_unit_rate, labour_rate, cost_unit_rate, as_built_quantity, as_built_unit_rate), variation_orders:elec_variation_orders(status, value)')
  ...
  const quotes: QuoteRow[] = (quotesRaw ?? []).map((q: any) => {
    ...
    // Mirror QuoteEditor itemTotal: only add labour_rate when cost_unit_rate is set
    const contract_value = lis.reduce((s: number, li: any) =>
      s + (li.quoted_quantity ?? 0) * (li.quoted_unit_rate ?? 0) + (li.cost_unit_rate != null ? (li.labour_rate ?? 0) : 0), 0)

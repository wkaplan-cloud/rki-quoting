...
// Same full SANS 10142-1 COC as the main COC section — shared COCModal.
export function JobCardCOCTab({ jobCardId, initialCOC, location, clientName, clientEmail, jobTitle }: Props) {
  const [coc, setCOC] = useState<ElecCOC | null>(initialCOC)
  const [open, setOpen] = useState(false)

  const hasCOC = !!coc
  const editingCOC = coc ?? newCOC(null, jobCardId, location ?? null, clientName ?? null, clientEmail ?? null, null)
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-xs mt-0.5 flex-wrap" style={{ color: S.muted }}>
              {hasCOC ? (
                ...
                <span>Create the SANS 10142-1 Certificate of Compliance for this job</span>
              )}
            </div>
          </div>

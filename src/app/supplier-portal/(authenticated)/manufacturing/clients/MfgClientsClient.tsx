...
import { Plus, X, Check, Archive, Pencil, User, Building2, ChevronRight } from 'lucide-react'
      ...
      {showForm && (
        <div className="rounded-2xl p-6 mb-6" style={{ background: S.card, border: `1.5px solid ${S.accent}`, boxShadow: '0 2px 12px rgba(27,79,138,0.1)' }}>
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-bold" style={{ color: S.text }}>{editingId ? 'Edit Client' : 'New Client'}</h3>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${S.border}` }}>
          {filtered.map((client, idx) => (
            <div key={client.id} className="flex items-center gap-4 px-5 py-4 transition-colors cursor-pointer"
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button onClick={e => { e.stopPropagation(); handleArchive(client.id) }} className="p-1.5 rounded-lg transition-colors"
                  style={{ color: S.muted }}
                  onMouseEnter={e => (e.currentTarget.style.background = S.input)}
                  onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                  <Archive size={13} />

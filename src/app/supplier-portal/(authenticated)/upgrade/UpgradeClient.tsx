...
import { Check, Zap, ChevronRight, Users, Briefcase, FolderOpen, HousePlug, Repeat } from 'lucide-react'
import { Suspense } from 'react'
import { planRank } from '@/lib/plan-features'

/** A plan as this screen shows it — the electrician tiers or the installer ones. */
...
const TIER_ICON = { starter: Users, professional: Briefcase, business: FolderOpen, installer: HousePlug, installer_pro: Repeat } as const
const TIER_COLOR = { starter: '#3A7CA5', professional: '#D9A441', business: '#166534', installer: '#1F5C45', installer_pro: '#B7862F' } as const
const TIER_BG   = { starter: 'rgba(58,124,165,0.08)', professional: 'rgba(217,164,65,0.08)', business: 'rgba(22,101,52,0.08)', installer: 'rgba(31,92,69,0.08)', installer_pro: 'rgba(183,134,47,0.08)' } as const

function UpgradeContent({ plans, installer }: { plans: readonly UpgradePlan[]; installer: boolean }) {
  const PLANS = plans
  ...
  const [selected, setSelected] = useState<string>(
    currentRank > 0
      ? (PLANS.find(p => planRank(p.id) > currentRank)?.id ?? PLANS[PLANS.length - 1].id)
      : installer ? 'installer_pro' : 'professional'
  )
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-widest mb-4"
            style={installer ? { background: 'rgba(31,92,69,0.1)', color: '#1F5C45' } : { background: 'rgba(58,124,165,0.1)', color: S.accent }}>
            {installer ? <HousePlug size={11} /> : <Zap size={11} />}
            QuotingHub for {installer ? 'Installers' : 'Electricians'}
          </div>
        ...
        {/* Tier cards */}
        <div className={`grid gap-4 mb-8 ${PLANS.length === 2 ? 'md:grid-cols-2 max-w-2xl mx-auto' : 'md:grid-cols-3'}`}>
          {PLANS.map(plan => {
            ...
            const isPopular = (plan.id === 'professional' || plan.id === 'installer_pro') && !isDisabled
...
export function UpgradeClient({ plans, installer }: { plans: readonly UpgradePlan[]; installer: boolean }) {
  return (
    <Suspense>
      <UpgradeContent plans={plans} installer={installer} />
    </Suspense>
  )
}

  ...
  try {
    ...
    const [{ data: slides }, { data: assets }, { data: specs }] = await Promise.all([
      ...
      supabase
        .from('studio_specs')
        .select(
          'id, board_id, org_id, slide_id, object_id, spec_name, description, notes, supplier_id, supplier_name, category, quantity, width, depth, height, materials, scatters, images, status, piece_id, item_specs'
        )
        .eq('board_id', boardId),
    ])

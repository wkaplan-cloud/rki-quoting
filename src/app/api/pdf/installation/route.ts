...
import { InstallationSheetPDF } from '@/lib/pdf/InstallationSheetPDF'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [{ data: project }, { data: lineItems }, { data: suppliers }, { data: settings }] = await Promise.all([
      ...
      supabase.from('settings').select('business_name, vat_rate').maybeSingle(),
    ])
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Installation-${project.project_number}.pdf"`,
      },
    })
  } catch (e) {
    return apiError(e)

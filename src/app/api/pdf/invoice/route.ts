...
export async function GET(req: NextRequest) {
  ...
  const buffer = await renderToBuffer(
    ...
    createElement(QuotePDF, { project, client: project.client ?? null, lineItems: lineItems ?? [], type: 'invoice', logoUrl, businessName: settings?.business_name, footerText: settings?.footer_text, termsConditions: settings?.terms_conditions }) as any
  )

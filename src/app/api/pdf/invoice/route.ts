...
export async function GET(req: NextRequest) {
  try {
    ...
    const [{ data: project }, { data: lineItems }, { data: settings }] = await Promise.all([
      ...
      supabase.from('settings').select('logo_url, business_name, business_address, vat_number, company_registration, bank_name, bank_account_number, bank_branch_code, footer_text, terms_conditions, deposit_percentage, vat_rate, quote_validity_days, payment_terms, lead_time').maybeSingle(),
    ])
    ...
    const buffer = await renderToBuffer(
      ...
      createElement(QuotePDF, { project, client: project.client ?? null, lineItems: lineItems ?? [], type: 'invoice', vatRate: (project as any).vat_rate ?? (settings as any)?.vat_rate ?? 15, logoUrl, businessName: settings?.business_name, businessAddress: settings?.business_address, vatNumber: settings?.vat_number, companyReg: settings?.company_registration, bankName: settings?.bank_name, bankAccount: settings?.bank_account_number, bankBranch: settings?.bank_branch_code, footerText: settings?.footer_text, termsConditions: settings?.terms_conditions, depositPct: project.deposit_percentage ?? settings?.deposit_percentage ?? 50, quotedDate: project.quoted_date ?? new Date().toISOString().split('T')[0], paymentTerms: settings?.payment_terms ?? null, leadTime: settings?.lead_time ?? null }) as any
    )

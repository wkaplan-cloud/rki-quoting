...
import { Trash2, ShieldCheck, X, AlertTriangle } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import toast from 'react-hot-toast'

type ModalStep = 'confirm' | 'mfa' | 'deleting'
...
export function DeleteStudioButton({ orgId, studioName }: { orgId: string; studioName: string }) {
  ...
  // Focus the code input whenever MFA step opens
  useEffect(() => {
    if (step === 'mfa') {
  ...
  async function verifyAndDelete() {
    ...
    // ── Step 2: MFA passed — delete the studio ─────────────────────────────
    setStep('deleting')
        ...
        >
          <div className="bg-[#1A1A18] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">
                    {step === 'mfa' ? 'Verify your identity' : step === 'deleting' ? 'Deleting…' : 'Delete studio'}
                  </p>
                  <p className="text-xs text-white/40 mt-0.5">
                    {step === 'mfa' ? 'Authenticator code required' : 'This cannot be undone'}
                  </p>
                </div>
              </div>
              {step !== 'deleting' && (
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3">
                  <p className="text-xs text-red-300 leading-relaxed">
                    This will delete the organisation, all team members, projects, clients, suppliers, settings, and associated accounts. <strong>There is no recovery.</strong>
                  </p>
                </div>
                <div className="flex gap-3 pt-1">
                  <button
                    onClick={close}
                    className="flex-1 py-2.5 rounded-lg border border-white/10 text-white/60 text-sm hover:bg-white/5 transition-colors cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
            ...
            {step === 'mfa' && (
              <div className="px-6 py-5 space-y-4">
                <p className="text-sm text-white/60 leading-relaxed">
                  Enter the 6-digit code from your authenticator app to confirm deletion of{' '}
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => { setStep('confirm'); setCode(''); setMfaError('') }}
                    disabled={loading}
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-lg bg-red-600 hover:bg-red-500 text-white text-sm font-medium transition-colors disabled:opacity-40 cursor-pointer flex items-center justify-center gap-2"
                  >
                    <ShieldCheck size={14} />

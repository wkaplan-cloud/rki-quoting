...
export function AppLayout({ children, isAdmin, businessName }: { children: React.ReactNode; isAdmin: boolean; businessName: string }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#F5F2EC]">
      ...
      {/* Mobile top bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#1A1A18] px-4 py-3 flex items-center gap-3 h-12">
        <button
        ...
        <img src="/logo.png" alt="QuotingHub" className="h-5 object-contain" style={{ filter: 'invert(1)' }} />
        {businessName && (
      ...
      <main className="md:ml-44 flex-1 flex flex-col min-h-screen min-w-0 overflow-x-clip pt-12 md:pt-0">
        {children}
      </main>
    </div>
  )

...
// same pattern as Assets/Specs. Every control writes straight to the store
// (instant live preview) and autosaves via setMasterLayout.
export function MasterThemePanel() {
  ...
  const enabled = masterLayout.enabled
  ...
  return (
    <div className="flex-shrink-0 w-[280px] h-full flex flex-col bg-[#F5F2EC] border-l border-[#D8D3C8]">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        <Row label="Enable Master Page">
          <Switch checked={enabled} onChange={v => setMasterLayout({ enabled: v })} label="Enable Master Page" />
        </Row>
        ...
        <Row label="Show Border" disabled={!enabled}>
          <Switch
            checked={masterLayout.showBorder}
            onChange={v => setMasterLayout({ showBorder: v })}
            disabled={!enabled}
            label="Show Border"
          />
        </Row>
        <Row label="Show Header" disabled={!enabled}>
          <Switch
            checked={masterLayout.showHeader}
            onChange={v => setMasterLayout({ showHeader: v })}
            disabled={!enabled}
            label="Show Header"
          />
        </Row>
        <Row label="Show Footer" disabled={!enabled}>
          <Switch
            checked={masterLayout.showFooter}
            onChange={v => setMasterLayout({ showFooter: v })}
            disabled={!enabled}
            label="Show Footer"
          />
        </Row>
        <Row label="Show Company Logo" disabled={!enabled || !masterLayout.showFooter}>
          <Switch
            checked={masterLayout.showLogo}
            onChange={v => setMasterLayout({ showLogo: v })}
            disabled={!enabled || !masterLayout.showFooter}
            label="Show Company Logo"
          />
        </Row>
        <Row label="Show Page Numbers" disabled={!enabled || !masterLayout.showFooter}>
          <Switch
        ...
        <label className="block">
          ...
          <input
            ...
            className="w-full text-[12px] px-2.5 py-2 rounded-md border border-[#D8D3C8] bg-white outline-none focus:border-[#9A7B4F] transition-colors text-[#2C2C2A] disabled:opacity-50"
          />
